...
class SupabaseDatabase {
  ...
  async addUser(user: Omit<User, "id" | "created_at">): Promise<User | null> {
    ...
    const { data, error } = await window.supabase.from("users").insert(newUser).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في إضافة المستخدم:", error)
      return null
    }
    return data
  }
  ...
  async updateUser(id: string, updates: Partial<User>): Promise<User | null> {
    const { data, error } = await window.supabase.from("users").update(updates).eq("id", id).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في تحديث المستخدم:", error)
      return null
    }
    return data
  }
  ...
  async deleteUser(id: string): Promise<boolean> {
    ...
    if (error) {
      console.error("[v0] ❌ خطأ في حذف المستخدم:", error)
      return false
    }
    return true
  }
  ...
  async addProductGroup(group: Omit<ProductGroup, "id" | "created_at">): Promise<ProductGroup | null> {
    ...
    const { data, error } = await window.supabase.from("product_groups").insert(newGroup).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في إضافة المجموعة:", error)
      return null
    }
    return data
  }
  ...
  async updateProductGroup(id: string, updates: Partial<ProductGroup>): Promise<ProductGroup | null> {
    const { data, error } = await window.supabase.from("product_groups").update(updates).eq("id", id).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في تحديث المجموعة:", error)
      return null
    }
    return data
  }
  ...
  async deleteProductGroup(id: string): Promise<boolean> {
    // حذف جميع المنتجات المرتبطة بهذه المجموعة
    await window.supabase.from("products").delete().eq("group_id", id)

    const { error } = await window.supabase.from("product_groups").delete().eq("id", id)
    if (error) {
      console.error("[v0] ❌ خطأ في حذف المجموعة:", error)
      return false
    }
    return true
  }
  ...
  async addProduct(product: Omit<Product, "id" | "created_at">): Promise<Product | null> {
    ...
    const { data, error } = await window.supabase.from("products").insert(newProduct).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في إضافة المنتج:", error)
      return null
    }
    return data
  }
  ...
  async updateProduct(id: string, updates: Partial<Product>): Promise<Product | null> {
    const { data, error } = await window.supabase.from("products").update(updates).eq("id", id).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في تحديث المنتج:", error)
      return null
    }
    return data
  }
  ...
  async deleteProduct(id: string): Promise<boolean> {
    ...
    if (error) {
      console.error("[v0] ❌ خطأ في حذف المنتج:", error)
      return false
    }
    return true
  }
  ...
  async addCustomer(customer: Omit<Customer, "id" | "created_at">): Promise<Customer | null> {
    ...
    const { data, error } = await window.supabase.from("customers").insert(newCustomer).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في إضافة العميل:", error)
      return null
    }
    return data
  }
  ...
  async updateCustomer(id: string, updates: Partial<Customer>): Promise<Customer | null> {
    const { data, error } = await window.supabase.from("customers").update(updates).eq("id", id).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في تحديث العميل:", error)
      return null
    }
    return data
  }
  ...
  async deleteCustomer(id: string): Promise<boolean> {
    ...
    if (error) {
      console.error("[v0] ❌ خطأ في حذف العميل:", error)
      return false
    }
    return true
  }
  ...
  async addOrder(order: Omit<Order, "id" | "created_at">): Promise<Order | null> {
    ...
    const { data, error } = await window.supabase.from("orders").insert(newOrder).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في إضافة الطلبية:", error)
      return null
    }
    return data
  }
  ...
  async updateOrder(id: string, updates: Partial<Order>): Promise<Order | null> {
    const { data, error } = await window.supabase.from("orders").update(updates).eq("id", id).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في تحديث الطلبية:", error)
      return null
    }
    return data
  }
  ...
  async deleteOrder(id: string): Promise<boolean> {
    // حذف جميع العناصر المرتبطة بهذه الطلبية
    await window.supabase.from("order_items").delete().eq("order_id", id)

    const { error } = await window.supabase.from("orders").delete().eq("id", id)
    if (error) {
      console.error("[v0] ❌ خطأ في حذف الطلبية:", error)
      return false
    }
    return true
  }
  ...
  async addOrderItem(item: Omit<OrderItem, "id" | "created_at">): Promise<OrderItem | null> {
    ...
    const { data, error } = await window.supabase.from("order_items").insert(newItem).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في إضافة عنصر الطلبية:", error)
      return null
    }
    return data
  }
  ...
  async addNotification(notification: Omit<Notification, "id" | "created_at">): Promise<Notification | null> {
    ...
    const { data, error } = await window.supabase.from("notifications").insert(newNotification).select().single()

    if (error) {
      console.error("[v0] ❌ خطأ في إضافة الإشعار:", error)
      return null
    }
    return data
  }
  ...
  async markNotificationAsRead(id: string): Promise<Notification | null> {
    ...
    if (error) {
      console.error("[v0] ❌ خطأ في تحديث الإشعار:", error)
      return null
    }
    return data
  }
  ...
  async deleteNotification(id: string): Promise<boolean> {
    const { error } = await window.supabase.from("notifications").delete().eq("id", id)
    if (error) {
      console.error("[v0] ❌ خطأ في حذف الإشعار:", error)
      return false
    }
    return true
  }
  ...
  async ensureDefaultAdmin(): Promise<void> {
    ...
    if (users.length === 0) {
      console.log("[v0] 🔧 إنشاء المدير الافتراضي...")
      await this.addUser({
        username: "admin",
        password: "admin",
        full_name: "المدير العام",
      ...
      console.log("[v0] ✅ تم إنشاء المدير الافتراضي")
    }

    const groups = await this.getProductGroups()
    if (groups.length === 0) {
      console.log("[v0] 🔧 إنشاء المجموعات الافتراضية...")
      ...
      if (group1 && group2 && group3 && group4) {
        console.log("[v0] 🔧 إضافة المنتجات الافتراضية...")
        ...
        console.log("[v0] ✅ تم إنشاء المجموعات والمنتجات الافتراضية")
      }
    }
  }
  ...
  async initializeDefaultData(): Promise<void> {
    try {
      console.log("[v0] 🔧 جاري التحقق من البيانات الافتراضية...")
      const users = await this.getUsers()

      // إضافة المدير الافتراضي
      if (users.length === 0) {
        console.log("[v0] 🔧 إضافة المستخدم الافتراضي...")
        await this.addUser({
          username: "admin",
          password: "admin",
          full_name: "المدير العام",
        ...
        console.log("[v0] ✅ تم إضافة المستخدم الافتراضي")
      }

      const groups = await this.getProductGroups()
      if (groups.length === 0) {
        console.log("[v0] 🔧 إضافة المجموعات الافتراضية...")
        const group1 = await this.addProductGroup({
          name: "مجموعة 1",
          description: "مجموعة المنتجات الأولى",
        })
        ...
        if (group1 && group2 && group3 && group4) {
          ...
          console.log("[v0] ✅ تم إضافة البيانات الافتراضية بنجاح")
        }
      }
    } catch (error) {
      console.error("[v0] ❌ خطأ في تهيئة البيانات الافتراضية:", error)
    }
  }
}
