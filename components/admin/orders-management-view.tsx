...
export function OrdersManagementView() {
  ...
  const handleMarkAsPrinted = (order: OrderWithRep) => {
    try {
      const allOrders = db.getOrders()
      const updatedOrders = allOrders.map((o) => (o.id === order.id ? { ...o, status: "printed" as const } : o))
      ...
      console.log("[v0] ✅ Order marked as printed, notification sent to:", order.sales_rep_id)

      setSelectedOrder(null)
      loadOrders()
      alert("تم تأكيد الطباعة وإرسال إشعار للمندوب بأن الطلبية جاهزة للاستلام ✅")
    } catch (error) {
