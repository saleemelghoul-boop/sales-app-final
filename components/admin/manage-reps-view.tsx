...
export function ManageRepsView() {
  ...
  const handleEditRep = (e: React.FormEvent) => {
    e.preventDefault()
    if (!editingRep) return

    try {
      db.updateUser(editingRep.id, {
  ...
  const toggleRepStatus = (repId: string, currentStatus: boolean) => {
    try {
      db.updateUser(repId, { is_active: !currentStatus })
      loadReps()
    } catch (error) {
